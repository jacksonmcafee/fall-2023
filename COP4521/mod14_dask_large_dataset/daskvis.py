...
Due Date: 12/3/2023 
About this project: This script visualizes the compute for daskload.
Assumptions: N/A
...
with ProgressBar():
    ...
    delayed(df.groupby(['Temperature(F)'])['Wind_Chill(F)'].min()).visualize("MinByTemp.png")
    delayed(df.groupby(['Temperature(F)'])['Wind_Chill(F)'].max()).visualize("MaxByTemp.png")
    delayed(df.groupby(['Temperature(F)'])['Wind_Chill(F)'].mean()).visualize("MeanByTemp.png")
